  ...
  for (const entry of edl) {
    if (entry.type === "source") {
      ...
      inputs.push({
        ...
        InputClippings: [
          {
            StartTimecode: secondsToTimecode(start),
            EndTimecode: secondsToTimecode(end),
          },
        ],
        VideoSelector: {},
        AudioSelectors: {
    ...
    } else if (entry.type === "fill" && entry.s3_key) {
      inputs.push({
        FileInput: `s3://${bucket}/${entry.s3_key}`,
        VideoSelector: {},
        AudioSelectors: {
          "Audio Selector 1": {
            DefaultSelection: "DEFAULT",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing export job ${job_id} for project ${projectId}`);
    ...
    try {
      // 3. Fetch metadata
      await updateJobProgress(serviceClient, job_id, 5);
      ...
      const edl = editDecision.edl_json as EdlEntry[];
      ...
      await updateJobProgress(serviceClient, job_id, 10);

      // 4. Build and submit MediaConvert job
      ...
      const inputs = buildMediaConvertInputs(edl, sourceVideoS3Uri, bucket);
      ...
      await updateJobProgress(serviceClient, job_id, 20);

      // 5. Poll for MediaConvert completion
      ...
      while (Date.now() - startTime < MAX_POLL_MS) {
        ...
        if (mappedProgress > lastProgress) {
          ...
          await updateJobProgress(serviceClient, job_id, mappedProgress);
        }
      ...
      const exportS3Key = `${exportS3KeyBase}.mp4`;
      ...
      const { data: exportRecord, error: exportError } = await serviceClient
        ...
        .insert({
          ...
          duration: 0, // MediaConvert doesn't easily expose this; could probe later
      ...
      await completeJob(serviceClient, job_id);
      ...
      console.error(`Export job ${job_id} failed:`, message);
      await failJob(serviceClient, job_id, editDecisionId, projectId, message);
      return errorResponse("export_failed", message, 500);
    }
  } catch (err) {
